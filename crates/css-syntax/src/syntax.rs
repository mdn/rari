...
#[cfg(test)]
fn css_ref_data() -> &'static css_syntax_types::WebrefCss {
    static DATA: std::sync::OnceLock<&'static css_syntax_types::WebrefCss> =
        std::sync::OnceLock::new();
    DATA.get_or_init(|| {
        let directory = tempfile::tempdir().expect("failed to create test data directory");
        rari_types::globals::DATA_DIR
            .set(directory.path().to_path_buf())
            .expect("test data directory must be initialized before loading CSS data");
        rari_deps::webref_css::update_webref_css(directory.path())
            .expect("failed to initialize test CSS data");
        // The loader retains the parsed data after the temporary directory is removed.
        rari_deps::webref_css::css_ref_data()
    })
}
