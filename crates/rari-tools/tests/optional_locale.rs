...
#[test]
fn absent_optional_german_skips_then_syncs_when_present() {
    ...
    let german = fixture.join("translated-content-de");
    ...
    assert!(Locale::translated().contains(&Locale::De));
    assert!(
        sync_translated_content(&[Locale::De], false)
            .unwrap()
            .is_empty()
    );
    assert!(!german.exists());

    write_doc(&german, Locale::De, "German", "OldWeb");
    fs::write(
        german.join("de/_wikihistory.json"),
        "{\"OldWeb\":{\"owner\":\"de\"}}\n",
    )
    .unwrap();
    assert!(
    ...
    assert!(german.join("de/web/index.md").exists());
    assert!(!german.join("de/oldweb/index.md").exists());
    assert!(
        fs::read_to_string(german.join("de/_redirects.txt"))
            .unwrap()
            .contains("/de/docs/OldWeb\t/de/docs/Web")
    );
    let history: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(german.join("de/_wikihistory.json")).unwrap())
            .unwrap();
